...
function record = func_em_missing_joint_abw_grad_ini(n_state, n_pt, n_event, n_feature, provided_label_mat,  feature_mat, event_label, Z_est, a_est, b_est, w_est)
...
% initialization the post prob mat
    ...
    % solve for solutions
    lb = [zeros(2*n_pt, 1); -inf*ones(n_pt*n_feature,1); 0.3];
    ub = [ones(2*n_pt, 1); inf*ones(n_pt*n_feature,1); 0.7];
...
%     [x, fval, exitflag] = fmincon(@(x)myfun_td_abg(x, n_pt, n_event, provided_label_mat, Z_est), x0, [], [], [], [], lb, ub, [], options); % Call solver
    ...
    % grad is provided
    options = optimset('Algorithm','trust-region-reflective',... % trust-region-reflective
    ...
    [func_val, grad_val] = func_obj_grad_abw(x0, n_pt, n_event, n_feature, provided_label_mat, feature_mat, Z_est)

    tic
    [x, fval, mflag, output] = fmincon(@(x)func_obj_grad_abw(x, n_pt, n_event, n_feature, provided_label_mat, feature_mat, Z_est),x0,[],[],[],[],lb,ub,[],options)
    time = toc
...
%     [x, fval] = fmincon(@(x)myfun_td_joint_global_w(x, n_pt, n_event, n_feature, provided_label_mat, feature_mat, Z_est), x0, [], [], [], [], lb(1:2*n_pt + n_feature), ub(1:2*n_pt + n_feature), [], options); % Call solver
    ...
    new_w_est = x(2*n_pt+1:2*n_pt + n_pt*n_feature)
    new_d_est = x(2*n_pt + n_pt*n_feature + 1) % sum(Z_est(2,:))/n_event;
    ...
    for ii = 1:n_pt
        for jj = 1:n_event
            if loc_cover_mat(ii, jj) == 0 % not a revealed loc
                loc_cover_mat(ii, jj) = sigmoid(feature_mat(jj,:,ii)*W(:,ii));
            end
        end
    end
