function f = myfun_td_joint_supervision(x, n_pt, n_event, n_feature, provided_label_mat, feature_mat, train_feature, Z_est)
...
for ii = 1:n_pt
    ...
    % the training features are non-empty
    if isfield(train_feature(ii), 'feature1')
        cur_n = size(train_feature(ii), 1);
        for kk = 1:cur_n
            f = f - 1/cur_n*gamma*(log(sigmoid(cur_w*train_feature(ii).feature1(kk,:).')) + log(1 - sigmoid(cur_w*train_feature(ii).feature0(kk,:).')) );
        end
    end
end
