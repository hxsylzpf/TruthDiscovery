function [record, min_fval, idx, fval_record, F_record] = func_em_missing_joint_abw_grad_ini_mul_run(n_state, n_pt, n_event, n_feature, provided_label_mat,  feature_mat, event_label, Z_est, a_est, b_est, w_est)
...
tic
cur_result = func_em_missing_joint_abw_grad_ini(n_state, n_pt, n_event, n_feature, provided_label_mat,  feature_mat, event_label, Z_est, a_est, b_est, w_est);
cur_result.time = toc;
...
for i = 1:n_run
    % new w_est
    W_est = w_c(i)*ones(n_feature, n_pt);
    w_est = W_est(:).';
    tic
    cur_result = func_em_missing_joint_abw_grad_ini(n_state, n_pt, n_event, n_feature, provided_label_mat,  feature_mat, event_label, Z_est, a_est, b_est, w_est);
    cur_result.time = toc;
