...
function record = func_em_missing_joint_abg_grad_ini(n_state, n_pt, n_event, provided_label_mat, event_label, Z_est, a_est, b_est, g_est, b_prior, g_prior)
...
%     [x, fval, exitflag] = fmincon(@(x)myfun_td_abg(x, n_pt, n_event, provided_label_mat, Z_est), x0, [], [], [], [], lb, ub, [], options); % Call solver
    ...
    x0 = [a_est b_est g_est d_est]  % Make a starting guess at the solution
    func_val = func_obj_grad_abg(x0, n_pt, n_event, provided_label_mat, Z_est, b_prior, g_prior)

    tic
    [x, fval, mflag, output] = fmincon(@(x)func_obj_grad_abg(x, n_pt, n_event, provided_label_mat, Z_est, b_prior, g_prior),x0,[],[],[],[],lb,ub,[],options)
    time = toc
...
%              if loc_cover_mat(i,j) ~= 0
                A(j) = A(j)*(g_est(j)*a_est(i))^provided_label_mat(i,j)*(1 - g_est(j)*a_est(i))^(1 - provided_label_mat(i,j));
                B(j) = B(j)*(g_est(j)*b_est(i))^provided_label_mat(i,j)*(1 - g_est(j)*b_est(i))^(1 - provided_label_mat(i,j));
%              end
        end
        % prob of being true
