...
function record = func_em_missing_joint_abg_ini(n_state, n_pt, n_event, provided_label_mat, event_label, Z_est, a_est, b_est, g_est)
...
%              if loc_cover_mat(i,j) ~= 0
                A(j) = A(j)*(a_est(i))^provided_label_mat(i,j)*(1 - g_est(j)*a_est(i))^(1 - provided_label_mat(i,j));
                B(j) = B(j)*(b_est(i))^provided_label_mat(i,j)*(1 - g_est(j)*b_est(i))^(1 - provided_label_mat(i,j));
%              end
        end
        % prob of being true
