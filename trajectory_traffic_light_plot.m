...
%% plot one pt's data
% to examine pt visited locs, pt revealed locs, and event locs
idx = 30;
my_figure(1/2.5, 1/4);
hold on
% first where he may visit
h1 = plot(data_store(idx).lat, data_store(idx).lon, '.', 'color', rgb('darkgray'));
% second event locations
h2 = plot(report_loc_filter(:,1), report_loc_filter(:,2), 'go', 'markersize', 10, 'linewidth', 2);
% third where he made reports
h3 = plot(data_store(idx).report_loc_filter(:,1), data_store(idx).report_loc_filter(:,2), 'b*', 'markersize', 8, 'linewidth', 2);

plot(37.758 + 0.002, -122.43 + 0.00752, '.', 'color', rgb('darkgray'));
plot(37.758+0.008, -122.43+0.0045, 'go', 'markersize', 10, 'linewidth', 2);
hold off

legend([h1 h2 h3], 'Visited locs', 'Event locs', 'Revealed locs');

% for 24
% x1 = 37.758; y1 = -122.425;
% x2 = x1 + 0.012; y2 = y1 + 0.014;

x1 = 37.758; y1 = -122.43;
x2 = x1 + 0.0122; y2 = y1+0.01;
...
set(gca, 'XTick', x1:0.002:x2, 'XTickLabel', 0:0.2:1.2, ...
    'YTick', y1:0.002:y2, 'YTickLabel', 0:0.2:1, 'fontsize', 16);
