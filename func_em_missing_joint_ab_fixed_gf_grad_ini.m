...
function record = func_em_missing_joint_ab_fixed_gf_grad_ini(n_state, n_pt, n_event, provided_label_mat, personal_loc_tend_mat, ...
...
%     [x, fval, exitflag] = fmincon(@(x)myfun_td_abg(x, n_pt, n_event, provided_label_mat, Z_est), x0, [], [], [], [], lb, ub, [], options); % Call solver
    ...
    options = optimset('Algorithm','interior-point', ... % trust-region-reflective, fin-diff-grads, interior-point, lbfgs
            'Display','off','GradObj','on', 'MaxIter', 300, 'TolFun', 1e-5, 'TolX', 1e-5, 'UseParallel','always'); % 'Hessian', 'lbfgs', 'UseParallel','always',
    
%     f_hess = @(x,lambda)func_hess_ab_fixed_gf(x,lambda,n_pt,n_event,provided_label_mat,personal_loc_tend_mat,Z_est,eta,a_prior,b_prior,g_val);
    
%     options = optimoptions(@fmincon,'Algorithm','interior-point', ... % trust-region-reflective, fin-diff-grads, interior-point, lbfgs
%             'Display','off','GradObj','on', 'MaxIter', 300, 'TolFun', 1e-5, 'TolX', 1e-5, 'UseParallel','always');
