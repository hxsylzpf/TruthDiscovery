...
function record = func_em_missing_joint_supervision_ini(n_state, n_pt, n_event, n_feature, provided_label_mat, feature_mat, event_label, loc_cover_mat, train_feature, ini_Z)
...
%         new_b_est(i) = (s(i)*n_event - loc_cover_mat(i,:).*provided_label_mat(i,:)*Z_est(2,:).')/(sum(loc_cover_mat(i,:)) - sum(loc_cover_mat(i,:).*Z_est(2,:)));
    ...
    cur_func_val = myfun_td_joint_supervision(x0, n_pt, n_event, n_feature, provided_label_mat, feature_mat, train_feature, Z_est)
    [x, fval] = fmincon(@(x)myfun_td_joint_supervision(x, n_pt, n_event, n_feature, provided_label_mat, feature_mat, train_feature, Z_est), x0, [], [], [], [], lb, ub, [], options); % Call solver
