function record = func_GLAD_grad_ini(n_state, n_pt, n_event, provided_label_mat, event_label, loc_cover_mat, Z_est)
...
% solve for a system of equations
    ...
    % grad is provided
    options = optimset('Algorithm','interior-point-convex', ... % trust-region-reflective, fin-diff-grads, interior-point, lbfgs
            'Display','off','GradObj','on', 'UseParallel','always');
        
x0 = [alpha_est beta0_est];  % Make a starting guess at the solution
func_val = func_obj_grad_glad(x0, n_pt, n_event, provided_label_mat, loc_cover_mat, z_post);

    tic
    [x, fval, mflag, output] = fminunc(@(x)func_obj_grad_glad(x, n_pt, n_event, provided_label_mat, loc_cover_mat, z_post),x0,options)
    time = toc
...
new_alpha_est = new_alpha_est/norm(new_alpha_est);
    ...
    if norm(new_alpha_est - alpha_est) + norm(new_beta0_est - beta0_est) < 5*1e-3
        break;
    else
