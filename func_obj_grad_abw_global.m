function [f, grad] = func_obj_grad_abw_global(x, n_pt, n_event, n_feature, provided_label_mat, feature_mat, Z_est)
...
% w - x(2*n_pt + 1) to x(2*n_pt + n_feature)
% d - x(2*n_pt + n_feature + 1)
...
w = x(2*n_pt + 1 : 2*n_pt + n_feature).';
for ii = 1:n_pt
    for jj = 1:n_event
        if loc_cover_mat(ii, jj) == 0
            loc_cover_mat(ii, jj) = sigmoid(feature_mat(jj,:,ii)*w);
        end
    end
end
...
for j = 1:n_event
    ...
    f1 = f1 + mylog(x(2*n_pt + n_feature + 1));
    f2 = f2 + mylog(1 - x(2*n_pt + n_feature + 1));
...
if nargout > 1
    grad_a = zeros(n_pt, 1);
    grad_b = zeros(n_pt, 1);
    grad_w = zeros(n_feature, 1); % each pt has a length-n_feature w_i
    grad_d = 0;
    ...
    for i = 1:n_pt
        for j = 1:n_event
            ...
            grad_w = grad_w + Z_est(2,j)*(provided_label_mat(i,j) - loc_cover_mat(i, j)*x(i))*(1 - loc_cover_mat(i, j))/(1 - loc_cover_mat(i, j)*x(i))*feature_mat(j,:,i).' ...
    ...
    for j = 1:n_event
        grad_d = grad_d + Z_est(2,j)/x(2*n_pt + n_feature + 1) - Z_est(1,j)/(1-x(2*n_pt + n_feature + 1));
    end
