function [w, w0] = func_tbpp_hidden_w(n_c, zeta, data)

% multiclass weighted logistic regression
...
% relation: p(target = k|data) = exp(wk*data+wk0)/sum_k' exp(wk'*data+wk'0)
    ...
    options = optimset('Algorithm','interior-point', ... % trust-region-reflective, fin-diff-grads, interior-point, lbfgs
        ...
        [x, fval, exitflag] = fminunc(@(x)myfun_obj_grad_tbpp_hidden_w(x, n_c, zeta, data), x0, options);
        w = x(1:n_c);
        w0 = x(n_c+1:2*n_c);
