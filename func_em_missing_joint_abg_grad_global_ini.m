...
function record = func_em_missing_joint_abg_grad_global_ini(n_state, n_pt, n_event, provided_label_mat, event_label, Z_est, a_est, b_est, g_est)
...
%     [x, fval, exitflag] = fmincon(@(x)myfun_td_abg(x, n_pt, n_event, provided_label_mat, Z_est), x0, [], [], [], [], lb, ub, [], options); % Call solver
    ...
    x0 = [a_est b_est g_est d_est]  % Make a starting guess at the solution
    func_val = func_obj_grad_abg_global(x0, n_pt, n_event, provided_label_mat, Z_est)

    tic
    [x, fval, mflag, output] = fmincon(@(x)func_obj_grad_abg_global(x, n_pt, n_event, provided_label_mat, Z_est),x0,[],[],[],[],lb,ub,[],options)
    time = toc
...
%     [x, fval] = fmincon(@(x)myfun_td_joint_global_w(x, n_pt, n_event, n_feature, provided_label_mat, feature_mat, Z_est), x0, [], [], [], [], lb(1:2*n_pt + n_feature), ub(1:2*n_pt + n_feature), [], options); % Call solver
    ...
    new_g_est = x(2*n_pt+1)
    new_d_est = x(2*n_pt+2) % sum(Z_est(2,:))/n_event;
    
        if norm(new_a_est - a_est) + norm(new_b_est - b_est) + norm(new_g_est - g_est)<= 5*1e-3
...
%              if loc_cover_mat(i,j) ~= 0
                A(j) = A(j)*(g_est*a_est(i))^provided_label_mat(i,j)*(1 - g_est*a_est(i))^(1 - provided_label_mat(i,j));
                B(j) = B(j)*(g_est*b_est(i))^provided_label_mat(i,j)*(1 - g_est*b_est(i))^(1 - provided_label_mat(i,j));
%              end
        end
        % prob of being true
