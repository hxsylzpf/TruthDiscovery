function record = func_pooledinvestment(n_pt, n_event, provided_info_mat, event_label, presence_mat)
...
% belief of claims
B = result_mv.ratio_ans;
% B = cell(n_event,1);
% % uniformly initialization
% for j = 1:n_event
%     B{j} = ones(size(uni_ans{j}));
% end
...
for j = 1:n_event
    ...
    for k = 1:cur_n_ans
       ...
       % find who also made this claim
       cur_source = (provided_info_mat(:,j) == cur_info);
       cur_h(k) = sum(cur_trust_vec(cur_source));
    end
    
    temp = sum(cur_h.^g);
    new_B{j} = cur_h.*cur_h.^g/temp;
    
%     for k = 1:cur_n_ans
%         new_B{j}(k) = cur_h(k)*cur_h(k)^g/temp;
%     end
